...
- Backend setup (uv): `cd backend && uv venv && source .venv/bin/activate && uv pip install -e .`.
- Backend tests: `cd backend && source .venv/bin/activate && uv run pytest`. Regenerate locks with `uv lock` when dependencies change.
- Frontend: `cd frontend && npm install && npm run dev` for local dev; `npm run build` for production bundle. Configure API origin via `VITE_API_BASE_URL` if not `http://localhost:8000`.
